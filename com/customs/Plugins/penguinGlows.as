class com.customs.Plugins.penguinGlows {
	...
	public function addPlayer(player_ob, targetX, targetY) {
		...
		if (player_ob.p_attributes.ng && player_ob.p_attributes.ng !== "0") {
			...
			_loc4.filters = [new flash.filters.GlowFilter(glows, 10, 1.7, 1.7, 15, 3, false, false)];
		}
	
		if (_loc3 >= 900000) {
			_loc4.textColor = "0xFFFFFF";
			_loc4.filters = [new flash.filters.GlowFilter(0x000000, 10, 1.7, 1.7, 15, 3, false, false)];
		}
		...
		_loc6.addEventListener(
			com.clubpenguin.hybrid.HybridMovieClipLoader.EVENT_ON_LOAD_INIT,
			com.clubpenguin.util.Delegate.create(this, ENGINE.onPlayerLoadInit, player_ob, _loc3, targetX, targetY)
		);
		_loc6.addEventListener(
			com.clubpenguin.hybrid.HybridMovieClipLoader.EVENT_ON_LOAD_START,
			com.clubpenguin.util.Delegate.create(this, ENGINE.onPlayerLoadStart, ENGINE.target_mc)
		);
		_loc6.addEventListener(
			com.clubpenguin.hybrid.HybridMovieClipLoader.EVENT_ON_LOAD_PROGRESS,
			com.clubpenguin.util.Delegate.create(this, ENGINE.onPlayerLoadProgress, ENGINE.target_mc)
		);

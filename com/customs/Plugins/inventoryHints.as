class com.customs.Plugins.inventoryHints {
	...
	public function showHint(mc, hint, isLocalized) {
		...
		var hintBox = INTERFACE.HINT;

		hintBox._x = pos.x;
		hintBox._y = pos.y - 28;
		hintBox.gotoAndStop(1);
		hintBox._visible = true;
		hintBox.message_txt.text = isLocalized ? hint : INTERFACE.getLocalizedString(hint);
		hintBox.message_txt._visible = false;
		hintBox.box_mc._visible = false;

		var messageWidth = hintBox.message_txt.textWidth;
		var frame = 1;

		hintBox.onEnterFrame = function() {
			if (frame >= 4 && frame <= 6) {
				hintBox.box_mc._visible = true;
				hintBox.box_mc._width = 66 + (frame - 4) * 15;
				hintBox.box_mc._height = 16 + (frame - 4) * 4;
			} else if (frame > 6) {
				hintBox.box_mc._width = (messageWidth > 72) ? messageWidth + 16 : 88;
				hintBox.box_mc._height = 22;
				hintBox.message_txt._visible = true;
				delete hintBox.onEnterFrame;
			}
			frame++;
		};
	}
